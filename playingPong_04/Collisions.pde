public static class Collisions {
  ...
  public static PVector getCollisionWithPad(Court c, Pad p, Ball ball) {
      ...
      PVector collision = getCollision(ball, p);
  ...
  public static PVector getCollision(Ball b, Pad p) {
    ...
    // la linea en la que juega el player
     ...
     Vect2[]player = new Vect2[ 2 ];

    player[ 0 ] = new Vect2((float) p.p.x, 0);
    player[ 1 ] = new Vect2((float) p.p.x, 1);

    PVector collision = new PVector();  // el centro de la cancha
    Vect2 intersectionPoint = Space2.lineIntersection( ball[ 0 ], ball[ 1 ], player[ 0 ], player[ 1 ] );
