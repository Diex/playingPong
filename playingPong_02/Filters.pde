...
void setupFilters(PImage background){
  
  filterW = background.width;
  filterH = background.height;
...
PImage getdiff(PImage video){    
  if (diffBuffer != null && video != null) { 
        ...
        diffBuffer.image(video, 0, 0,bg.width, bg.height); // como uso image() el filtro tiene que ser TEXTURE (es razonable ya que yo no estoy creando ninguna geometría)        
        diffBuffer.endDraw();
